...
logger = logging.getLogger("openbrain.books_sheet_sync")

BOOKS_SHEET_NAME = "Books"
BOOKS_HEADERS = ["id", "title", "content", "topic", "who", "created_at", "Problem"]
END_COL_LETTER = chr(ord("A") + len(BOOKS_HEADERS) - 1)  # "G"
BOOKS_SNAPSHOT_FIELDS = ["title", "content", "topic", "who"]
BOOKS_TAB_NAME = "books"

# 1-indexed column positions
COL = {h: i + 1 for i, h in enumerate(BOOKS_HEADERS)}
...
class BookSyncResult:
    ...
    def as_message(self) -> str:
        msg = f"Books synced: {self.updated} updated, {self.skipped} skipped"
        if self.errors:
...
def _open_books_worksheet(spreadsheet: gspread.Spreadsheet) -> gspread.Worksheet:
    try:
        return spreadsheet.worksheet(BOOKS_SHEET_NAME)
    except gspread.WorksheetNotFound:
        ws = spreadsheet.add_worksheet(title=BOOKS_SHEET_NAME, rows=500, cols=len(BOOKS_HEADERS))
        logger.info("Created new worksheet: %s", BOOKS_SHEET_NAME)
        return ws
    ...
    requests = [
            ...
            "addConditionalFormatRule": {
                "rule": {
                    "ranges": [{
                        ...
                        "startColumnIndex": 0, "endColumnIndex": len(BOOKS_HEADERS),
                    }],
                    "booleanRule": {
                        "condition": {
                            "type": "CUSTOM_FORMULA",
        ...
        # Expand grid to accommodate all columns BEFORE any repeatCell/validation requests
        {
            "updateSheetProperties": {
                "properties": {
                    "sheetId": sid,
                    "gridProperties": {"columnCount": len(BOOKS_HEADERS)},
                },
        ...
        "setBasicFilter": {
            "filter": {
                "range": {
                    ...
                    "endRowIndex": 500,
                    "startColumnIndex": 0,
                    "endColumnIndex": len(BOOKS_HEADERS),
                }
            }
        }
    })
...
def _fetch_books_from_db() -> list[dict[str, Any]]:
...
def _row_to_sheet_values(book: dict[str, Any]) -> list[str]:
    return [
        str(book["id"]),
        book.get("title") or "",
        book.get("content") or "",
        book.get("topic") or "",
        book.get("who") or "",
        _format_dt(book.get("created_at")),
        book.get("sync_error") or "",
    ]
...
def _push_books_to_sheet(worksheet: gspread.Worksheet, books: list[dict[str, Any]]) -> int:
    worksheet.clear()
    rows = [BOOKS_HEADERS] + [_row_to_sheet_values(b) for b in books]
    worksheet.update(f"A1:{END_COL_LETTER}{len(rows)}", rows, value_input_option="RAW")
    with engine.begin() as conn:
        write_snapshots(conn, BOOKS_TAB_NAME, "entries", books, BOOKS_SNAPSHOT_FIELDS)
    logger.info("Pushed %d book rows to Books tab", len(books))
    return len(books)
...
def _pull_books_from_sheet(worksheet: gspread.Worksheet) -> BookSyncResult:
    """
    Read Books tab and write back user edits to the DB via 3-way merge.
    ...
    header = values[0]
    try:
        idx = {h: header.index(h) for h in BOOKS_HEADERS}
    ...
    with engine.begin() as conn:
        db_rows = conn.execute(
            text("""
                SELECT id, title, content, topic, who, created_at, sync_error
                FROM entries WHERE type = 'book'
            """)
        ).mappings().all()
        snapshots = fetch_snapshots(conn, BOOKS_TAB_NAME, row_ids)
    ...
    for row_num, row in enumerate(sheet_rows, start=2):
        ...
        if not db:
            logger.warning("Books tab row %d: id %d not in DB, skipping", row_num, entry_id)
            result.skipped += 1
            continue
        ...
        db_values = {field: db.get(field) for field in BOOKS_SNAPSHOT_FIELDS}
        ...
        for field in BOOKS_SNAPSHOT_FIELDS:
        ...
        if conflict_fields:
            ...
            conflicts.append((entry_id, f"Books tab conflict -- {msg}"))
    ...
    if pending_updates or conflicts or clear_ids:
        with engine.begin() as conn:
            ...
            for update_values in pending_updates:
                ...
                if set_clauses:
                    conn.execute(
                        text(f"UPDATE entries SET {', '.join(set_clauses)} WHERE id = :id AND type = 'book'"),
                        params,
                    )
    ...
    if pending_updates:
        logger.info("Reverse synced %d book rows from sheet via 3-way merge", len(pending_updates))
    if conflicts:
        logger.warning("Skipped %d book rows due to sheet/DB conflicts", len(conflicts))
    return result
...
def sync_books_tab() -> BookSyncResult:
    """
    Full Books tab sync: pull -> push.
    ...
    worksheet = _open_books_worksheet(spreadsheet)

    try:
        existing_header = worksheet.row_values(1)
    except Exception:
        existing_header = []
    needs_init = (existing_header[: len(BOOKS_HEADERS)] != BOOKS_HEADERS
                  or len(existing_header) != len(BOOKS_HEADERS))
    ...
    result = BookSyncResult()
    if not needs_init:
        result = _pull_books_from_sheet(worksheet)

    books = _fetch_books_from_db()
    result.pushed = _push_books_to_sheet(worksheet, books)
