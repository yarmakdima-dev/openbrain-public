...
logger = logging.getLogger("openbrain.projects_sheet_sync")

PROJECTS_SHEET_NAME = "Projects"
PROJECTS_HEADERS = ["id", "title", "content", "status", "topic", "who", "created_at", "updated_at", "Problem"]
END_COL_LETTER = chr(ord("A") + len(PROJECTS_HEADERS) - 1)  # "I"
PROJECTS_SNAPSHOT_FIELDS = ["status", "title", "content", "topic", "who"]
PROJECTS_TAB_NAME = "projects"

VALID_STATUSES = {"open", "done", "archived"}

# 1-indexed column positions
COL = {h: i + 1 for i, h in enumerate(PROJECTS_HEADERS)}
...
class ProjectSyncResult:
    ...
    def as_message(self) -> str:
        msg = f"Projects synced: {self.updated} updated, {self.skipped} skipped"
        if self.errors:
...
def _open_projects_worksheet(spreadsheet: gspread.Spreadsheet) -> gspread.Worksheet:
    try:
        return spreadsheet.worksheet(PROJECTS_SHEET_NAME)
    except gspread.WorksheetNotFound:
        ws = spreadsheet.add_worksheet(title=PROJECTS_SHEET_NAME, rows=500, cols=len(PROJECTS_HEADERS))
        logger.info("Created new worksheet: %s", PROJECTS_SHEET_NAME)
        return ws


def _apply_sheet_formatting(spreadsheet: gspread.Spreadsheet, worksheet: gspread.Worksheet) -> None:
    """Freeze header, status dropdown, widths, read-only cues, and filters."""
    sid = worksheet.id
    ...
    requests = [
            ...
            "addConditionalFormatRule": {
                "rule": {
                    "ranges": [{
                        ...
                        "startColumnIndex": 0, "endColumnIndex": len(PROJECTS_HEADERS),
                    }],
                    "booleanRule": {
                        "condition": {
                            "type": "CUSTOM_FORMULA",
        ...
        # Expand grid to accommodate all columns BEFORE any repeatCell/validation requests
        {
            "updateSheetProperties": {
                "properties": {
                    "sheetId": sid,
                    "gridProperties": {"columnCount": len(PROJECTS_HEADERS)},
                },
        ...
        # created_at / updated_at / Problem: grey background as read-only visual cue
        {
            "repeatCell": {
                "range": {
                    "sheetId": sid, "startRowIndex": 1, "endRowIndex": 500,
        ...
        "setBasicFilter": {
            "filter": {
                "range": {
                    ...
                    "endRowIndex": 500,
                    "startColumnIndex": 0,
                    "endColumnIndex": len(PROJECTS_HEADERS),
                }
            }
        }
    })
...
def _fetch_projects_from_db() -> list[dict[str, Any]]:
    with engine.begin() as conn:
        rows = conn.execute(
            text("""
                SELECT id, title, content, status, topic, who, created_at, updated_at, sync_error
                FROM entries
                WHERE type = 'project'
                ORDER BY
                    CASE status WHEN 'open' THEN 0 WHEN 'done' THEN 1 ELSE 2 END ASC,
...
def _row_to_sheet_values(project: dict[str, Any]) -> list[str]:
    return [
        str(project["id"]),
        ...
        project.get("who") or "",
        _format_dt(project.get("created_at")),
        _format_dt(project.get("updated_at")),
        project.get("sync_error") or "",
    ]
...
def _push_projects_to_sheet(worksheet: gspread.Worksheet, projects: list[dict[str, Any]]) -> int:
    worksheet.clear()
    rows = [PROJECTS_HEADERS] + [_row_to_sheet_values(p) for p in projects]
    worksheet.update(f"A1:{END_COL_LETTER}{len(rows)}", rows, value_input_option="RAW")
    with engine.begin() as conn:
        write_snapshots(conn, PROJECTS_TAB_NAME, "entries", projects, PROJECTS_SNAPSHOT_FIELDS)
    logger.info("Pushed %d project rows to Projects tab", len(projects))
    return len(projects)
...
def _pull_projects_from_sheet(worksheet: gspread.Worksheet) -> ProjectSyncResult:
    """
    Read Projects tab and write back user edits to the DB via 3-way merge.

    Editable fields: status, title, content, topic, who.
    Read-only fields: id, created_at, updated_at, sync_error.
    ...
    header = values[0]
    try:
        idx = {h: header.index(h) for h in PROJECTS_HEADERS}
    ...
    with engine.begin() as conn:
        db_rows = conn.execute(
            text("""
                SELECT id, title, content, status, topic, who, created_at, updated_at, sync_error
                FROM entries WHERE type = 'project'
            """)
        ).mappings().all()
        snapshots = fetch_snapshots(conn, PROJECTS_TAB_NAME, row_ids)
    ...
    for row_num, row in enumerate(sheet_rows, start=2):
        ...
        if not db:
            logger.warning("Projects tab row %d: id %d not in DB, skipping", row_num, entry_id)
            result.skipped += 1
            continue
        ...
        db_values = {field: db.get(field) for field in PROJECTS_SNAPSHOT_FIELDS}
        ...
        for field in PROJECTS_SNAPSHOT_FIELDS:
        ...
        if conflict_fields:
            ...
            conflicts.append((entry_id, f"Projects tab conflict -- {msg}"))
        ...
        with engine.begin() as conn:
            ...
            for update_values in pending_updates:
                ...
                if set_clauses:
                    ...
                    conn.execute(
                        text(f"UPDATE entries SET {', '.join(set_clauses)} WHERE id = :id AND type = 'project'"),
                        params,
                    )
    ...
    if pending_updates:
        logger.info("Reverse synced %d project rows from sheet via 3-way merge", len(pending_updates))
    if conflicts:
        logger.warning("Skipped %d project rows due to sheet/DB conflicts", len(conflicts))
    return result
...
def sync_projects_tab() -> ProjectSyncResult:
    """
    Full Projects tab sync: pull -> push.
    ...
    worksheet = _open_projects_worksheet(spreadsheet)

    try:
        existing_header = worksheet.row_values(1)
    except Exception:
        existing_header = []
    needs_init = (existing_header[: len(PROJECTS_HEADERS)] != PROJECTS_HEADERS
                  or len(existing_header) != len(PROJECTS_HEADERS))
    ...
    result = ProjectSyncResult()
    if not needs_init:
        result = _pull_projects_from_sheet(worksheet)

    projects = _fetch_projects_from_db()
    result.pushed = _push_projects_to_sheet(worksheet, projects)
